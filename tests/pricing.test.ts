...
describe('pricing calculator estimates', () => {
    it('respects included guests and addons', () => {
        ...
        expect(estimate.baseTotal).toBe(2920);
        expect(estimate.enhancementsTotal).toBe(480);
        expect(estimate.total).toBe(3400);
        expect(estimate.deposit).toBe(1000);
    });
    ...
    it('computes default deposit when not provided', () => {
        ...
        expect(estimate.deposit).toBe(option.deposit);
    });
});
